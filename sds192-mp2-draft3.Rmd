...
makePlot <- function(candidatePAC,name) {
  candidatePAC %>%
    ...
    geom_bar(stat = "identity")+labs(title=paste("Commitee Contribution towards ", if(name == "P80003353"){"Romney"}else{"Obama"}), x = "Committee", y = "Contribution Amount ($)")+
...
RomneyCommiteeContributions <- committeeContribution(RomneyContributions)
ObamaCommiteeContributions <- committeeContribution(ObamaContributions)
```


```{r fig.width=9, fig.height=9, message=FALSE, warning=FALSE}
#Create plots for RomneyPAC and ObamaPAC
makePlot(RomneyCommiteeContributions, Romney)
makePlot(ObamaCommiteeContributions, Obama)
```
